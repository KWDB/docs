...
KWDB 提供丰富的时序特色查询能力，显著降低查询分析难度。支持用户根据业务需求自定义函数，并内置丰富的时间日期函数、数学函数、聚合函数（`MIN`、`MAX`、`AVG` 等）和窗口函数。针对时序场景，提供最新值查询、值过滤查询、条件查询、插值查询、补值查询等专属查询方式，全面满足时序数据分析需求。
...
### 实时数据压缩
KWDB 支持实时数据压缩，能够根据数据类型自动适配最优压缩算法，具备 5-30 倍数据压缩能力，降低数据存储带来的成本。用户还可在创建时序表时按列独立设置编码方式、压缩算法和压缩级别，也可在表创建后按需调整，或通过集群参数统一配置默认压缩策略，灵活平衡压缩效率与查询性能。
