...
用户在 EMQX Dashboard 配置 KWDB 的连接和请求信息后，EMQX 即可自动将接收到的 MQTT 消息转化为包含 `INSERT` 语句的 HTTP 请求，通过 RESTful API 的 Insert 接口写入 KWDB 中的指定数据表。有关 Insert 接口的具体信息，参见 [Insert 接口](../development/restful-api/connect-restful-api.md#insert-接口)。
