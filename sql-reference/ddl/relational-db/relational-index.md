...
KWDB 支持为表创建标准索引和倒排索引。创建索引后，系统可以使用索引快速定位数据而无需查看表的全部数据，有助于提高数据库的性能。
...
- 标准索引
    ...
    ![](../../../static/sql-reference/CKldbkkszo37Ukx23MqcbUnxnog.png)

- 倒排索引

    ![](../../../static/sql-reference/Ypzlb7onyo0BxTxtDQuck7UZndJ.png)
...
| `interleave_clause` | 支持使用交错索引（Interleaving Indexes）优化查询性能，这会改变 KWDB 存储数据的方式。  |
...
- 更改索引名称。
    ...
    ![](../../../static/sql-reference/XzRNbXkoyovt9MxvleHcSdDfnGc.png)
...
| 参数 | 说明 |
| --- | --- |
| `IF EXISTS` | 可选关键字。当使用 `IF EXISTS` 关键字时，如果目标索引存在，系统修改目标视图。如果目标索引不存在，系统修改目标索引失败，但不会报错。当未使用 `IF EXISTS` 关键字时，如果目标索引存在，系统修改目标索引。如果目标索引不存在，系统报错，提示目标索引不存在。 |
| `table_name` | 索引所在表的名称。 |
| `index_name` | 当前索引的名称。|
...
`DROP INDEX` 语句用于删除表的索引信息。

### 所需权限

用户拥有索引所属表的 CREATE 权限。

### 语法格式

![](../../../static/sql-reference/GQoSbNGteozN6hxBOGicJ7aunhe.png)
