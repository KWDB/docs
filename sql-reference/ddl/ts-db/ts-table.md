...
`SHOW CREATE <table>` 语句用于查看当前或指定数据库下指定表的建表语句，如未指定数据库则默认为当前数据库。
...
- 查看当前或指定数据库下指定表的建表语句。如未指定，则默认使用当前数据库。
    ...
    ![](../../../static/sql-reference/JaazbY1m4oRFq5x59BWcQpkrnTg.png)
...
- 添加、删除列或标签时，需要应用停机，以确保结构变更的原子性、一致性，避免潜在的一致性问题。
...
![](../../../static/sql-reference/KpxFbeOMLoLjjbxAG6jc9OdwnTf.png)
