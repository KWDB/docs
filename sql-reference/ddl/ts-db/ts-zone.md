...
集群启动时，会自动生成预配置的副本区域和 `default` 副本区域。其中预配置的副本区域适用于内部系统数据，`default` 副本区域适用于集群中的其他数据。用户可以直接使用或调整上述副本区域，也可以为指定数据库、表、分区单独设置副本区域。
...
- **数据库**：为指定数据库单独设置副本区域。
- **表**：为指定时序表单独设置副本区域。
- **分区**：为指定时序表的分区单独设置副本区域。

数据复制过程中，副本区域的优先级排序为 分区 > 表 > 数据库 > 集群。
...
- 修改系统数据库或系统数据数据分片：用户是 `admin` 角色的成员。默认情况下，`root` 用户属于 `admin` 角色。
- 修改其他库或其他库下的数据分片、分区或表：用户是 `admin` 角色的成员或者拥有目标对象的 CREATE 权限或 ZONECONFIG 权限。默认情况下，`root` 用户属于 `admin` 角色。
...
- 修改数据库的区域配置
     ...
     以下示例将 `vtx` 数据库的副本数改为 5 个。

     ```sql
     ALTER DATABASE vtx CONFIGURE ZONE USING num_replicas = 5;
     CONFIGURE ZONE 1
...
- 修改表的区域配置
     ...
     以下示例将 `vehicles` 表的副本数改为 3 个。

     ```sql
     ALTER TABLE vehicles CONFIGURE ZONE USING num_replicas = 3;
     CONFIGURE ZONE 1
