...
每个数据分片都隶属于一个特定的副本区域（zone）。集群在重新平衡数据分片时，会考虑副本区域的配置，以确保遵守所有约束条件。副本区域更多信息见[副本区域](./ts-zone.md)。
...
除了用户可见的数据库和表之外，KWDB 在以下系统数据分片存储了部分内部数据，进行了区域配置：
...
- `system`：包括分配新表 ID 所需的信息以及追踪集群节点状态，副本数设置为 5，以提高容错性。
- `timeseries`：包含集群监控数据。
