...
| `where_clause` | 可选参数，数据删除操作的筛选语句。如未指定，默认删除表内数据列的所有数据，保留表的标签数据。`WHERE` 子句支持以下格式：<br>- `where <timestamp_column> = <value>`：只删除指定时间范围内的数据列数据，保留标签数据。其中，时间戳列必须是数据所在表的第一列，对应的数值只支持时间戳常量，例如 `'2023-12-01 08:00:00'`。KWDB 支持指定一个或多个时间范围，支持使用 `<`、`>`、`=`、`!=`、`>=`、`<=`、`and`、`or`、`in` 和 `not in` 操作符。<br>- `where <primary_tag> = <value>`：同时删除数据列数据和标签数据。其中，`value` 只支持常量。如需删除所有数据列数据和标签数据时，当待删除数据的时序表拥有多个主标签，需要列出所有主标签和标签值，用 `and` 连接，例如 `ptag1 = 1 and ptag2 = 2`。也支持使用 `or` 操作符，但是连接的必须是两个完整的主标签筛选条件，例如 `(ptag1 = value1 and ptag2= value2) or (ptag1 = value3 and ptag2= value4)`。如需删除指定的数据列数据和标签数据，可以列出部分主标签和标签值，用 `and` 连接，例如 `ptag1 = 1 and ptag2 = 2`。指定部分主标签和标签时，不支持使用 `and` 连接相同主标签的不同标签值，例如 `ptag1 = 1 and ptag1 = 2`，也不支持使用 `or` 连接，例如 `ptag1 = 1 or ptag2 = 2` 或者 `ptag1 = 1 or ptag1 = 2`。目前，标签列不支持使用 `<`、`>`、`!=`、`>=`、`<=`、`in` 和 `not in` 操作符。 <br>- `where <timestamp_column> = <value> and <primary_tag> = <value>`：只删除指定时间范围内的数据列数据，保留标签数据。|
