...
| `client_encoding` | 客户端字符集编码，支持以下参数值设置（不区分大小写）：<br>- `utf8` 或 `utf-8`<br/>- `gbk`<br>- `gb18030`<br> **注意**：通过 ODBC 连接 KaiwuDB 时，ODBC 驱动会根据系统的字符编码自动设置字符集。如果需要使用与系统字符集不同的字符集编码，则需手动进行设置。 | `utf8` |
