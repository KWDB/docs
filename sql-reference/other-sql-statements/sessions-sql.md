...
| `client_encoding` | 客户端字符集编码，支持以下参数值设置（不区分大小写）：<br>- `utf8` 或 `utf-8`<br/>- `gbk`<br>- `gb18030`<br>- `big5`<br>**注意**：在 Windows 系统下通过 ODBC 连接 KWDB 时，由于 ODBC 驱动仅支持 UTF-8，无需手动设置此参数，系统将默认使用 UTF-8 编码，并自动处理其他编码。 | `utf8` |
