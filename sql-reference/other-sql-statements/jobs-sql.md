...
| `job_type`             | 任务类型，包括：<br >- UNSPECIFIED（无法明确归类任务） <br >- CHANGEFEED（变更数据捕获） <br >- IMPORT（导入）<br >- EXPORT（导出）<br >- SCHEMA CHANGE（模式更改）<br >- SCHEMA CHANGE GC（模式更改垃圾回收） <br >- CREATE_STATS（创建统计信息）<br >- AUTO_CREATE_STATS（自动创建统计信息）<br >- REPLICATION_STREAM（流计算副本任务） <br >- REPLICATION_INGESTION（数据拉取副本任务）<br >- RESTART（历史重启）<br >- SYNC_META_CACHE（元数据缓存同步）<br >- STREAM（流计算）|
