...
| 运算符      | 描述                                                                                                                                                    |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CAST(...)   | 类型转换 <br >**说明** <br >- 时序表建表时，如果时间戳列的数据类型设置为 TIMESTAMP，系统会自动处理为 TIMESTAMPTZ，对该列的转换将按照数据库设置的时区进行转换。<br >- 使用 `CAST` 函数将 CHAR、NCHAR 或 VARCHAR 数据类型转换为长度不同的 CHAR、NCHAR 或 VARCHAR 数据类型时，如果目标长度不匹配原始字符串的实际字节数，服务端可能会返回乱码，客户端（如 KWDB JDBC 或 KWDB 开发者中心）会提示报错。 |
...
| time_bucket(timestamp_column: timestamp, interval: STRING) → timestamp           | 时间桶函数，支持时间戳对齐。<br> 参数说明：<br>- timestamp_column：时间戳列。 <br>- interval：时间间隔（单位：秒）。例如 '10s'。                                                                                                                                                |
| time_bucket(timestamp_column: timestamptz, interval: STRING) → timestamptz       | 时间桶函数，支持时间戳对齐。<br> 参数说明：<br>- timestamp_column：时间戳列。 <br>- interval：时间间隔（单位：秒）。例如 '10s'。                                                                                                                                                |
...
::: warning 说明
在 ARM 环境中使用 `ABS` 函数时，避免传入非整数的参数。
:::
