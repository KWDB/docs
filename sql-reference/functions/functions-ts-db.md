...
| first(* [, order_by_col]) → any element | 返回条件范围内时间戳最小的行中所有列的值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| first(target_col [, order_by_col]) → target_col | 返回条件范围内时间戳最小的行中，指定目标列的值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| first_row(* [, order_by_col]) → any element | 返回条件范围内时间戳最小的行中所有列的值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| first_row(target_col [, order_by_col]) → target_col | 返回条件范围内时间戳最小的行中，指定目标列的值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| first_row_ts(* [, order_by_col]) → timestamptz | 返回条件范围内时间戳最小的行的时间戳值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| first_row_ts(target_col [, order_by_col]) → timestamptz | 返回条件范围内时间戳最小的行中，指定目标列所在行的时间戳值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| firstts(* [, order_by_col]) → timestamptz | 返回条件范围内时间戳最小的行的时间戳值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| firstts(target_col [, order_by_col]) → timestamptz | 返回条件范围内时间戳最小的行中，指定目标列所在行的时间戳值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| interpolate(method, mode) | 补值函数，需与 `time_bucket_gapfill()` 配合使用，补值结果类型与原始列类型一致。<br>参数说明：<br>- `method`：补值算法，仅支持数值型聚合函数。<br>- `mode`：补值模式，取值包括 `prev`（前值）、`next`（后值）、`linear`（线性）、`null` 或常量值。 |
| last(* [, order_by_col]) → any element | 返回条件范围内时间戳最大的行中所有列的值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| last(target_col [, order_by_col]) → target_col | 返回条件范围内时间戳最大的行中，指定目标列的值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| last(target_col, deadline) → target_col | 返回指定截止时间前时间戳最大的行中，指定目标列的值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `deadline`：截止时间，仅支持时间戳常量格式。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| last_row(* [, order_by_col]) → any element | 返回条件范围内时间戳最大的行中所有列的值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| last_row(target_col [, order_by_col]) → target_col | 返回条件范围内时间戳最大的行中，指定目标列的值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| last_row_ts(* [, order_by_col]) → timestamptz | 返回条件范围内时间戳最大的行的时间戳值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| last_row_ts(target_col [, order_by_col]) → timestamptz | 返回条件范围内时间戳最大的行中，指定目标列所在行的时间戳值（含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| lastts(* [, order_by_col]) → timestamptz | 返回条件范围内时间戳最大的行的时间戳值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
| lastts(target_col [, order_by_col]) → timestamptz | 返回条件范围内时间戳最大的行中，指定目标列所在行的时间戳值（不含 NULL）。<br>支持在子查询、关联查询（JOIN）、嵌套查询（FROM 子查询）、WHERE、GROUP BY、HAVING、ORDER BY 等多种查询场景中使用，支持与 KWDB 现有的聚合函数联合使用。<br>参数说明：<br>- `target_col`：目标列，支持 FLOAT4、FLOAT8、INT2、INT4、INT8、STRING、TIMESTAMP、TIMESTAMPTZ、VARBYTES、VARCHAR。<br>- `order_by_col`：可选，排序用时间戳列，仅支持 `TIMESTAMPTZ`；未指定时，使用第一列时间戳列排序。<br>**当前限制**：<br>- 仅支持在 SELECT 列表中使用，不可作为普通列进行计算；<br>- 仅支持单列作为参数，传入数值常量、操作符、表达式或函数将触发语法错误。 |
...
| STATE_WINDOW (val: varchar)        | 按设备状态划分窗口，连续相同状态的数据归于同一窗口，状态变化时窗口关闭。具体操作说明，参见[分组窗口查询](../dml/ts-db/ts-select.md#分组窗口查询)。    |
