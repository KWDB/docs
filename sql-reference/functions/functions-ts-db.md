...
| time_bucket_gapfill(timestamp_column: timestamp, interval: STRING) → timestamp     | 时间桶函数，根据时间间隔对时间戳进行对齐，并填充缺失的时间戳行。该函数需要与 `GROUP BY` 配合使用。使用 `time_bucket_gapfill()` 时，如果查询的其他列不在 `GROUP BY` 指定的范围内，需要使用聚合函数来处理这些列。<br>`time_bucket_gapfill()` 函数也可以和 `interpolate()` 函数配合使用，填充缺失的时间戳行的同时对指定数据列进行补值。<br>更多信息，参见[插值查询](../../db-administration/data-query/ts-data-query.md#插值查询)。<br> 参数说明：<br>- timestamp_column：时间戳列。 <br>- interval：时间间隔，支持的单位包括秒（s）、分（m）、小时（h）、天（d）、周（w）、月（mon）、年（y）。目前，KaiwuDB 不支持复合时间格式，如 `1d1h`。 |
