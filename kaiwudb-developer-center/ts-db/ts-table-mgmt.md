...
- 查看时序表
  - 用户为 Admin 用户、Admin 角色成员或拥有目标表的任意权限。  
- 删除时序表
  - 用户为 Admin 用户、Admin 角色成员或拥有 TABLE DROP 权限。
- 重命名时序表
  - 用户为 Admin 用户、Admin 角色成员或拥有目标表的 DROP 权限及所在数据库的 CREATE 权限。
  - 待重命名数据库不是当前使用的数据库。
- 从时序表中导出数据
...
#### 修改数据活跃时间、生命周期、分区间隔和描述信息

如需修改时序表的数据活跃时间、生命周期、分区间隔或描述信息，遵循以下步骤。

1. 在数据库导航区，双击需要修改的时序表，或者右键单击需要修改的时序表，然后选择**编辑时序表**。

2. 在属性页签根据需要修改数据活跃时间、生命周期、分区间隔和描述信息，然后单击**保存**。
