...
::: warning 提示
KWDB 单节点部署支持 [基于 DRBD 的主备复制](../best-practices/single-ha-drbd.md)，如果您计划实施高可用方案，请先参阅相关文档。
:::
...
| 连接方式 | 特点 | 适用场景 |
|---------|------|---------|
| **[kwbase CLI 工具](./access-kaiwudb/access-kaiwudb-cli.md)** | 内置命令行工具，支持安全和非安全模式，适合自动化脚本 | 命令行操作、自动化脚本、运维管理 |
| **[KaiwuDB JDBC](./access-kaiwudb/access-kaiwudb-jdbc.md)** | 标准 JDBC 接口，支持连接池，适合 Java 应用集成 | Java 应用程序开发、生产环境集成 |
| **[KaiwuDB 开发者中心](./access-kaiwudb/access-kaiwudb-kdc.md)** | 图形化管理界面，直观易用，支持可视化操作 | 可视化管理、数据浏览、查询调试 |

## 卸载 KWDB

如需了解如何卸载 KWDB，请参见[卸载 KWDB 数据库](../deployment/uninstall-cluster.md)。
