...
5. 安装程序自动生成配置文件模板并打开编辑器。根据实际环境修改各参数，保存并退出后，安装程序将自动开始安装。
    ...
    | `brpc_port` | 时序引擎间的数据传输端口，单节点部署时系统会自动忽略该设置，默认为 `27257`。 |
...
8. 完成安装后，根据提示选择是否立即启动数据库：

    ```plain
    是否启动数据库：(y/N)
    ```

    - 输入 `y`：系统自动启动并初始化 KWDB。
    - 输入 `N`：跳过启动，后续需手动启动 KWDB。

        ```bash
        systemctl start kaiwudb
        ```

9. 在主功能菜单，输入 `5` 退出部署流程。
