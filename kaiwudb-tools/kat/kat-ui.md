...
KAT 支持创建新会话以及对历史会话进行查看、分享、重命名和删除操作。
...
- 如需通过 KAT 部署 KWDB，需在 KWDB 安装部署前提前配置数据库连接信息。
...
2. 单击**添加配置**。然后在弹出的对话框中，添加数据库信息。
    ...
    - `启用数据库`：配置是否启用该数据库连接。勾选后，系统启用该数据库。通过 KAT 部署或连接 KWDB 时，需确保该选项已启用。
