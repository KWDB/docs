...
KaiwuDB 智能体工具 （英文名称 KaiwuDB Agent Tools, 简称 KAT）是一款基于 [MCP](https://modelcontextprotocol.io/introduction)（Model Context Protocol，模型上下文协议）协议的智能助手，专为 KaiwuDB 用户打造。KAT 将自然语言处理与数据库深度结合，让用户通过简单的对话即可完成产品使用智能问答、自动化安装部署、自然语言查询分析、故障诊断和性能调优。通过借助 LLM（Large Language Model，大语言模型）智能提示能力、结合知识库和向量搜索技术，KAT 有效降低 KaiwuDB 的学习、使用和运维成本，提升数据交互效率，助力企业轻松驾驭数据洞察。
...
- CMD MCP Server
  ...
  - 支持读取系统级资源占用情况。
- KWDB MCP Server
  - [KWDB MCP Server](../../development/kwdb-mcp-server/connect-kwdb-mcp-server.md) 是一个基于 [MCP](https://modelcontextprotocol.io/introduction) 协议的服务器实现，它通过 MCP 协议提供一套工具和资源，用于与 KaiwuDB 数据库交互和提供商业智能功能。
