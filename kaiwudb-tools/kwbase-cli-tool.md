...
| `--use-raft-store`      | 启用时序分布式日志存储引擎。该引擎适用于分布式集群的时序数据场景，可在机械硬盘环境下提升数据写入性能。启用后，系统会在数据目录的时序引擎目录下创建 `raftlog` 子目录，用于存储和管理分布式日志。<br>**注意**：<br>- 该参数必须在数据库**首次安装启动时**指定，默认为关闭状态。<br>- 数据库初始化完成后，**无法**通过修改启动参数切换存储引擎。|
