...
- **问题描述**
  ...
  集群中节点状态异常后，手动重启节点失败，提示`fatal error`。

- **问题解答**

  如果集群正在进行数据写入，且异常节点正在执行生命周期任务，可能会导致异常节点重启失败。可以通过以下步骤排查节点是否正在执行生命周期任务，如果是，需要先删除相应的分区，然后再重启节点。

  1. 查看 `logs` 目录下的 `TsEngine.log`，确认是否有 “delete expired data start” 但没有 “succeeded” 或 “failed” 日志，这表明该表正在处理，需要手动干预。

     ```Bash
     -- 开始执行
     I240920 db.cpp:740 table[79] delete expired data start, expired data end time[16762710]
     
     -- 执行成功/失败
     I240920 db.cpp:748 table[79] delete expired data succeeded
     I240920 db.cpp:748 table[79] delete expired data failed
     ```

  2. 在当前节点的存储目录，根据日志中的表 ID，即示例中的 `[79]` 找到对应表的数据目录。
     ...
     kwbase-data
         |-----tsdb
                |-----79  #表79的数据目录
                      |-----1   # entity group 1
                            |-----79_1   # sub entity group 1
                                    |-----16675200  # 分区目录，以时间戳命名
                                    |-----16761600
                                    |-----16934400
                            |-----79_2   # sub entity group 2
                                    |-----16675200  # 分区目录，以时间戳命名
                                    |-----16761600
                                    |-----16934400
                      |-----2   # entity group 2
  ...
  3. 从大到小遍历表数据目录下所有 entity group 中 subgroup 的分区目录，查找第一个时间戳 ≤ 开始执行日志中 `end_time` 的分区，以该分区为基准，删除所有时间戳小于该时间戳的分区目录。

     1. 确认待删除的分区目录：

         示例中的`end_time`为 16762710，按照时间戳的从大到小顺序，找到第一个时间戳 ≤ `end_time`的分区为16761600，因此需要删掉所有小于 16761600 时间戳的分区，即示例中的 16675200、16675200 和 16675200 分区目录。
     ...
     2. 删除分区目录。如果在删除时遇到“只读文件系统”错误，需使用`umount`命令卸载相关目录（需要 sudo 或 root 权限），然后再次执行删除操作。

         示例：

        ```Bash
        $ rm -rf 16675200
        rm: 无法删除 '16675200/1/79.0': 只读文件系统
        rm: 无法删除 '16675200/1/79.1': 只读文件系统
        rm: 无法删除 '16675200/1/79.2': 只读文件系统
        rm: 无法删除 '16675200/1001/79.0': 只读文件系统
        rm: 无法删除 '16675200/1001/79.1': 只读文件系统
        rm: 无法删除 '16675200/1001/79.2': 只读文件系统
        rm: 无法删除 '16675200/2001/79.0': 只读文件系统
        rm: 无法删除 '16675200/2001/79.1': 只读文件系统
        rm: 无法删除 '16675200/2001/79.2': 只读文件系统
        
        
        $ umount 16675200/1
        $ umount 16675200/1001
        $ umount 16675200/2001
        $ rm -rf 16675200
        ```
...
- **问题解答**
    ...
    4. 在 KaiwuDB DataX Utils 所在目录，执行数据迁移命令时，设置 JVM 参数，增加内存。
        ...
        ```shell
        java -jar -DyamlPath=<yml_path> -DdataxPath=<datax_path> -Dpython=<python>  -Darguments="$bin_path --jvm=\"-Xms2G -Xmx4G\"" kaiwudb-datax-utils-2.1.0.jar
        ```
