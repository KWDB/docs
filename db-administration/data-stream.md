...
- 流计算配置
  - 断点数据处理策略：当用户启动一个处于停止状态的流计算时，系统会检查是否存在断点数据（未处理数据）并使用流计算最低水位线标识断点数据的范围。如果断点数据的数据量未超过内置阈值，则系统采用同步方式优先获取并处理这部分数据，然后再开始处理实时数据。如果断点数据超过内置阈值，则系统采用异步方式处理这部分数据。用户也可以通过 `PROCESS_HISTORY` 参数不处理这部分数据。
