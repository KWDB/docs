...
| `index_def` | 可选项，定义索引列表，支持定义一个或多个索引，各索引之间使用逗号（`,`）隔开。每个索引必须指定要索引的列，可选择指定名称。 索引名称在表中必须唯一，并且[遵循数据库标识符规则](../../../sql-reference/sql-identifiers.md)。有关如何新建索引的详细信息，参见[新建索引](./index-mgmt-relational.md#新建索引)。|
...
- 修改现有表上的主键列。
- 设置现有表的区域。
- 重命名现有表。表的重命名支持跨数据库迁移操作，即重命名后的表可迁移到新的数据库和新的模式中。
