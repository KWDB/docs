...
- 查看指定数据分片、分区、数据库或表的副本区域信息

    ```sql
    SHOW ZONE CONFIGURATION FOR [RANGE <range_name> | DATABASE <database_name> | TABLE <table_name> | PARTITION <partition_name> OF TABLE <table_name> ];
    ```
...
- 查看指定数据库的副本信息
     ...
     以下示例查看 `d1` 数据库的副本区域信息。
     
     ```sql
     SHOW ZONE CONFIGURATION FOR DATABASE d1;
     ```
...
- 修改数据库、表、数据分片和分区的区域配置
- 移除数据库、表、数据分片和分区的区域配置
- 对指定数据库、表或数据分片进行手动均衡
...
```sql
ALTER [DATABASE <database_name> | TABLE <table_name> | RANGE <range_name> | PARTITION <partition_name> OF TABLE <table_name> ] CONFIGURE ZONE [USING <variable> = [COPY FROM PARENT | <value>], <variable> = [<value> | COPY FROM PARENT], ... | USING REBALANCE | DISCARD];
```
