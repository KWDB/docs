...
- 开启多线程导入操作后，如果导入的 `.csv` 文件中带有换行符，会导致导入失败。此时，用户应删除导入的数据库或者表，将 `thread_concurrency` 参数取值设置为 1， 然后再重新导入文件。
...
- 导入关系表
  ...
  - 同时导入用户数据和元数据
    ...
    ```sql
    IMPORT TABLE CREATE USING "<sql_path>" CSV DATA ("<file_path>") WITH [DELIMITER = '<char>' | comment | charset = '<coding>'];
    ```

  - 只导入用户数据

    ```sql
    IMPORT INTO <table_name> CSV DATA ("<file_path>") WITH [ DELIMITER = '<char>' | charset = '<coding>'];
    ```
...
| `enclosed`           | 可选参数，导入时序数据时，用于指定包围符。默认为双引号（`"`），支持单引号（`'`）。使用单引号（`'`）作为包围符时，格式为 `"'"`。使用双引号（`"`）作为包围符时，格式为 `'"'`。包围符不能与分隔符相同。注意：导入带有包围符的换行符时，应避免开启多线程导入操作。|
...
| `charset` | 可选参数，用于指定待导入数据的字符集编码。默认值为 `utf8`，支持指定为 `gbk`、`gb18030`、`big5` 或 `utf8`。<br>**注意**：指定的字符集编码必须与实际字符集编码一致，否则可能会导致数据导入失败或数据乱码问题。|
...
- 导入关系数据库
    ...
    ```sql
    IMPORT DATABASE CSV DATA ("<db_path>") WITH [DELIMITER = '<char>' | comment | charset = '<coding>'];
    ```
