...
- **表数据**：导入指定时序表、稀疏时序表或关系表的元数据、权限信息和用户数据。
...
- 稀疏表导入时，需将csv文件中的 `""` 进行处理，仅保留非空的列。

:::

## 表级别数据导入

KWDB 支持通过 SQL 语句将时序表或关系表的数据从一个 KWDB 数据库导入到另一个 KWDB 数据库。导入内容包括元数据和权限信息（存储在 `meta.sql` 文件中）以及用户数据（存储在 `.csv` 文件中）。

此外，KWDB 还支持直接导入 TDengine 数据库导出的 CSV 文件，导入时系统会自动删除文件中填充的空格，无需手动处理。导入前需注意以下几点：
- 删除首行（列名行）：TDengine 导出的 CSV 文件首行为列名，导入前需手动删除。
- 数据类型映射：TDengine 与 KWDB 的数据类型不完全对应，建表时需进行替换。例如，TDengine 的 `BINARY` 类型可替换为 KWDB 的 `VARCHAR` 类型。
- 空值处理：需在 `IMPORT` 语句中指定 `NULLIF='NULL'`。
...
| 参数                 | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `sql_path`           | 待导入元数据文件的存放路径，支持 `nodelocal://node_id/folder_name/meta.sql` 和 `server_ip/folder_name/meta.sql` 两种格式。<br > `nodelocal://node_id/folder_name/meta.sql`：导入本地节点的元数据文件。<br > - `node_id`：节点 ID。当本地只有一个节点时，`node_id` 取值是 `1`。<br > - `folder_name`：存放导入元数据文件的文件夹名称，默认是 `/var/lib/kaiwudb/extern/<folder_name>`。<br > `server_ip/folder_name/meta.sql`：导入指定服务器的元数据文件。<br > - `server_ip`：指定服务器的 IP 地址和端口号，例如 `http://172.18.0.1:8090`。<br >- `folder_name`：存放导入元数据文件的文件夹名称。|
