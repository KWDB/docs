...
KWDB 支持在创建或修改时序表时，为每个数据列单独指定编码算法、压缩算法及压缩级别，针对不同数据特征选择最优压缩策略，在存储空间和系统资源之间灵活权衡。
...
各级压缩配置的优先级从高到低为：**列级自定义配置** > **数据类型默认值** > **cluster setting 全局配置**。建议优先通过 cluster setting 设置全局基准，仅对有特殊需求的列单独指定配置。
...
| `ts.compress.algorithm` | 全局默认压缩算法，支持 `lz4`、`zstd`、`zlib`、`snappy`。优先级低于列级配置。 | `lz4` | string |
