...
部署 KWDB 集群后，用户可以通过修改启动参数、CPU 资源占用率或实时集群参数，自定义集群配置：
...
3. 根据需要，在 `KAIWUDB_START_ARG` 开头的启动命令后添加或修改启动参数和参数值。
...
2. 根据需要，修改 KWDB 的 CPU 资源占用率（`CPUQuota`）。

    以下示例将 CPU 资源占用率设置为 `180%`。
...
| `server.rest.infulxdb.batch_params`                                   | 无模式写入接口需设置批量写入参数<batch_enabled>;<batch_size>;<workers>;<flush_interval>,默认值为 `false;100;16;1000` <br>**说明**<br>`batch_enabled`表示：是否打开批量写入<br> `batch_size`表示：每批次最大行数，达到后立即刷新<br>`workers`表示：并发写入worker数量<br>`flush_interval`表示：定时刷新间隔，单位为：`ms`                                                                                                                                                                                                                                                                                                         | -        | string      |
...
| `sql.pg_encode_short_circuit.enabled`                   | 控制是否将 PG 协议编码逻辑转移至时序引擎执行。启用后，查询结果的编码将由时序引擎承担，以降低大结果集场景下主引擎的编码序列化开销，提升查询性能。支持以下取值：<br>- `true`：启用短路路径，由时序引擎执行 PG 协议编码。<br>- `false`：禁用短路路径，回退至主引擎标准编码。 | `true` | bool |
...
| `sql.ts_insert_select.block_memory`                       | 时序数据 `INSERT INTO SELECT` 语句单次写入的数据块内存限制，该参数不能设置为0。                                                                                                                                                                                                                                                                                                                                                                       | `200`       | int      |
...
| `ts.compress.last_segment.enabled` | 控制是否对 last segment 启用压缩。支持以下设置值：<br>- `true`：启用压缩。last segment 数据写入时进行压缩，可减少内存和磁盘占用，但会增加写入时的 CPU 消耗。<br>- `false`：禁用压缩。last segment 数据以原始格式存储，写入性能更高，但占用更多内存和磁盘空间。<br>该参数用于在写入性能和存储效率之间进行权衡。写入密集型场景建议设置为 `false` 以提高写入吞吐量；存储敏感型场景建议设置为 `true` 以减少资源占用。 | `true` | boolean |
| `ts.compress.stage` | 控制时序数据的压缩模式。支持以下设置值：<br>- `0`：关闭编码，关闭压缩。<br>- `1`：开启编码，关闭压缩。<br>- `2`：关闭编码，开启压缩。<br>- `3`：开启编码，开启压缩。<br>**说明：** 从 3.1.x 版升级到 3.2.0 版本后，如果升级前手动设置过 `ts.compress.stage=2`，升级后需手动改为 `ts.compress.stage=3`。如未手动设置过，则无影响。 | `3` | int |
| `ts.compress.algorithm` | 设置全局默认压缩算法。支持 `lz4`、`zstd`、`zlib`、`snappy`、`disabled`。优先级低于列级自定义压缩配置，更多信息，参见[数据压缩](./storage-mgmt.md#数据压缩) 。| `lz4` | string |
| `ts.compress.level` | 设置全局默认压缩级别。支持 `low`、`medium`、`high`。优先级低于列级自定义压缩配置，更多信息，参见[数据压缩](./storage-mgmt.md#数据压缩) 。 | `medium` | string |
| `ts.dedup.rule`                                         | 数据去重策略。支持设置为以下参数：<br>- `override`：整行去重，后写入的数据覆盖已写入的具有相同时间戳的数据。<br>- `merge`：对相同时间戳的数据进行去重和整合。同一时间戳的数据多次写入时，后写入的非 NULL 列值会覆盖先前写入的对应列值，最终整合为一行记录。该模式适用于相同时间戳不同字段数据分批写入的场景。<br>- `discard`：忽略新写入的重复数据，保留已写入的数据。当重复数据写入失败后，客户端会收到成功插入数据的条数，并以 Notice 的形式展示未成功插入数据的条数。| `override`  | string   |
...
| `ts.raft_log.sync_period` | 时序数据 raft log 落盘周期。默认 `0s` 表示实时落盘，设置为非零值时，系统将按指定时间间隔强制同步落盘。延长落盘时间有助于提升写入性能，但如果节点意外停机，可能丢失最多一个周期内的数据。适用于高性能要求、对数据一致性要求相对较低的场景。 | `0s` | duration |
| `ts.raftlog_combine_wal.enabled` | 启用时序数据 raft log 和 WAL 合并功能，开启后可减少写放大效应，提升时序数据写入性能。<br>**说明：**<br>- 仅支持在无时序表时设置为 `true`；存在时序表后可设置为 `false`，但不可再设置为 `true`。设置为 `true` 后需重启集群生效。<br>- 创建时序表并写入数据后，如需完全使用 WAL 或完全关闭 WAL，可将该参数设置为 `false`，设置 WAL 级别参数，然后重启集群。<br>- 机器故障（如断电）导致节点下线后，不建议修改该参数，否则可能丢失数据。 | `false` | bool |
...
| `ts.rows_per_block.max_limit`                           | 控制 Entity Segment 中单个 Block 可保存的最大行数，避免单个 Block 过大导致读取效率下降。<br>取值范围为 [1, 50000]。<br>该参数用于调节存储密度和查询性能之间的平衡。增大此值可提高压缩率，节省存储空间（适合存储优先场景），减小此值可减少单次查询扫描的数据量，提升查询响应速度（适合查询优先场景）。                                                                                                                                                                                                                             | `4096`      | int      |
| `ts.rows_per_block.min_limit`                           | 控制 Entity Segment 中单个 Block 可保存的最小行数，即触发数据从 Last Segment 合并到 Entity Segment 的行数阈值。<br>取值范围为 [1, 50000]。<br>该参数确保每个 Block 包含足够的数据行以提高压缩效率和存储密度，避免生成过多小 Block 导致文件碎片化和元数据开销增加。<br>增大此值可减少 Block 数量，降低元数据开销，但会延长数据合并时间；减小此值可加快数据持久化，但可能产生更多小 Block，增加文件碎片。                                                                                                                                             | `512`      | int      |
...
| `cluster.connection.timezone`               |设置集群默认时区。<br>支持数字偏移，例如：`（8,-5）`<br>时区名，例如：`（Asia/Shanghai）`<br>ISO8601格式，例如：`（+8:00）`<br>系统表：`kwdb_internal.cluster_settings` <br>**说明**<br>删除原参数：`server.restful_service.default_reguest_timezone`<br>也可替换原参数`server.restful_service.default_reguest_timezone`为新参数：`cluster.connection.timezone`替换后restful连接设置全局时区使用时，原时区处理逻辑不变。 | `UTC` | timestamp        |