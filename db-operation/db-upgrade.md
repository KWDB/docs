...
| 部署拓扑 | 当前版本 | 目标版本 | 升级方式 |
|---------|---------|---------|---------|
| 单机部署 | 3.0.0、3.1.0 、3.2.0| 3.2.1 | 脚本和命令行部署：安装程序<br>容器镜像部署：容器镜像升级 |
|  | 2.x | 3.2.1 | 导入导出 |
| 多副本集群 | 3.0.0、3.1.0、3.2.0 | 3.2.1 | 脚本和命令行部署：安装程序<br>容器镜像部署：容器镜像升级 |
|  | 2.x | 3.2.1 | 导入导出 |
| 单副本集群 | 3.0.0、3.1.0、3.2.0 | 3.2.1 | 脚本和命令行部署：安装程序<br>容器镜像部署：容器镜像升级 |
|  | 2.x | 3.2.1 | 导入导出 |

:::warning 说明
- 从 3.0.0 升级至 3.2.0 时，如需使用 3.1.0 以后新增的时序 raft log 存储引擎，请先部署全新集群，再导入历史数据。
- 3.2.0 版本将 `ts.raft_log.sync_period` 默认值从 `10s` 改为 `0s`（实时落盘）。**多副本集群**从 3.0.0 或 3.1.0 升级至 3.2.0 前，需要提前在旧版本节点将该参数切换为 `0s` 并等待生效，否则可能导致升级异常。具体操作，参见[集群部署升级准备](#集群部署)中的相关步骤。
:::
...
2. 如果是多副本集群，在升级前完成以下操作：
    ...
    1. 将 `ts.raft_log.sync_period` 切换为 `0s`：
...
11. 完成所有节点升级后，验证数据是否完整。多副本集群还需查询分片状态，确认集群状态正常。
...
10. 完成所有节点升级后，验证集群数据是否完整。多副本集群还需查询分片状态，确认集群状态正常。
