...
| --------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `timestamp` | timestamptz | The absolute time when the message occurs.                                                                                                                                                                                                               |
...
| `client_encoding` | The character encoding of the clients. Available options (case-insensitive): <br>- `utf8` or `utf-8`<br/>- `gbk`<br>- `gb18030`<br> **Note** <br> - When connecting to KWDB using ODBC, the ODBC driver will automatically set the character set based on the character encoding of the system. To use a different character encoding you need to manually set a character encoding. <br> - When connecting to KWDB using the CLI tool, avoid using the `VARCHAR[]` data type when using the GB18030 character encoding. | `utf8` |
...
| `experimental_enable_hash_sharded_indexes` | Enables or disables the hash sharded index feature for time-series tables. When enabled, the system uses a hash algorithm to evenly distribute index key values across multiple shard buckets for improved data distribution.<br>Accepted values:<br>- `on` or `true` (enables the feature)<br>- `off` or `false` (disables the feature)<br>For more information, see [Partitions](../ddl/ts-db/ts-partition.md). | `off` |
...
- Set a session variable to a single value.
