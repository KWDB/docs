...
The `SHOW RANGES` statement is used to display range information for databases, tables, and indexes, verifying how SQL data maps to underlying ranges and the location of replicas.
...
- Show ranges for a table.
    ...
    The following example shows the ranges for the `orders` table.
...
- Show ranges for an index.
    ...
    The following example shows the ranges for the `primary` index of the `orders` table.
...
- Show ranges for a database.
    ...
    The following example shows the ranges for the `db3` database.
...
- Modify the zone configuration of a system range

    The following example changes the number of replicas to `7` for the `meta` range.
