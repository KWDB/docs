...
This section describes how to deploy a KWDB cluster on a single node by using Docker Run commands. Note: For production environments, it is recommended to deploy only one node per machine to improve availability and reduce the risk of data loss.

## Prerequisites

- The [KWDB container image](../cluster-prepare.md#container-images) is obtained.
...
- One of the following user permissions:
  - Root user access or regular user with `sudo` privileges:
    - Users with passwordless `sudo` won't need to enter passwords during deployment.
    - Users without passwordless `sudo` will be prompted for passwords when elevating privileges.
  - If the installation user is not the root user, run `sudo usermod -aG docker $USER` to add the user to the `docker` group.

## Steps

1. (Optional) If you need to deploy the KWDB cluster in secure mode, run the following commands to create the certificate authority, the client certificate for the `root` user, and the node server certificate.

    ::: warning Note

    If deploying in secure mode across machines, you need to use the `./kwbase cert create-node <node_ip>` command to create certificates and keys for all nodes, and transfer all the certificates and keys to all nodes.
    ...
    - `-v`: Sets up a volume mount, mounting the host's `/etc/kaiwudb/certs` directory to the container's `<certs_dir>` directory for certificate and key storage.
    ...
    - `bash -c`: Executes the following certificate creation commands within the container:
      ...
      - All commands use `--certs-dir=<certs_dir>` to specify the certificate storage directory and `--ca-key=<certs_dir>/ca.key` to specify the CA key path.
...
2. Start three or more database instances.
    ...
    - `-v`: Sets up volume mounts:
      - Mounts the host's `/var/lib/kaiwudbX` directory to the container's `/kaiwudb/deploy/kwdb-container` directory for persistent data storage.
      - In secure mode, mounts the host's `/etc/kaiwudb/certs` directory to the container's `<certs_dir>` directory for certificate and key storage.
    ...
    - `./kwbase start`: Database startup command. The flags differ depending on the mode:
      - `--insecure`: (Insecure mode only) Runs in insecure mode.
      - `--certs-dir=<certs_dir>`: (Secure mode) Specifies the certificate directory location.
      ...
      - `--brpc-addr=:2725X`: brpc communication port between time-series engines for inter-node communication.
      - `--http-addr=0.0.0.0:8080`: Address and port the HTTP interface listens on.
      - `--store=/kaiwudb/deploy/kwdb-container`: Specifies the data storage location.
...
3. Initialize the cluster:
    ...
    - `./kwbase init`: Executes the cluster initialization command.
      - `--insecure`: (Insecure mode only) Specifies insecure mode.
      - `--certs-dir=<certs_dir>`: (Secure mode) Specifies the certificate directory location.
