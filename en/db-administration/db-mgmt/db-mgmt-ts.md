...
```sql
CREATE TS DATABASE [IF NOT EXISTS] <database_name> [RETENTIONS <keep_duration>] [PARTITION INTERVAL <interval>] [COMMENT [=] <'comment_text'>];
```
...
| `database_name` | The name of the database to create, which must be unique and follow these [Identifier Rules](../../sql-reference/sql-identifiers.md). Currently, the name does not support Chinese characters and supports up to 63 bytes.|
...
- Change the zone configurations of the database
  ...
  ```sql
  ALTER DATABASE <database_name> CONFIGURE ZONE 
...
| `new_name` | The new name of the database, which must be unique and follow these [Identifier Rules](../../../en/sql-reference/sql-identifiers.md). Currently, the name does not support Chinese characters and supports up to 63 bytes.|
...
- Change the zone configurations of a database.
    ...
    This example sets the number of the replicas of the `tsdb` database to `5`.
