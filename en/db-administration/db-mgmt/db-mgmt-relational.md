...
| `db_name` | The name of the database to create, which must be unique and follow these [Identifier Rules](../../../en/sql-reference/sql-identifiers.md). |
...
This example sets the `db1` database to the current database.
...
- Modify zones for other databases: the user must be a member of the `admin` role or have been granted `CREATE` or `ZONECONFIG` privileges on the specified database(s). By default, the `root` user belongs to the `admin` role.
...
| `new_name` | The new name of the database, which must be unique and follow these [Identifier Rules](../../../en/sql-reference/sql-identifiers.md).|
