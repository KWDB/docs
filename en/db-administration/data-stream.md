...
- Stream computing objects
  ...
  - Support the use of stream computing's real-time computation function by setting `SYNC_TIME=0` and `LOW_LATENCY='ON'`, with real-time writing enabled by default.
...
- Stream computing configurations
  ...
  - Processing policy for out-of-order data: When creating a stream, you can specify how long a stream waits for data through the `SYNC_TIME` parameter. By default, it is set to `1m`. Only the out-of-order data that exceeds this time can be computed by the stream.  If the `SYNC_TIME` parameter is set to `0`, the process will skip the sorting process, and the out-of-order data in the aggregate stream computation will be directly treated as expired data. The out-of-order data in the normal stream computation does not affect the final result.
