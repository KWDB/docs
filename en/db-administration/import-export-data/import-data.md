...
| `thread_concurrency` | Optional. Specify the number of data to be written concurrently. KWDB splits, concurrently reads, and writes imported files based on this setting. By default, it is set to `1`. The value should be greater than `0` and smaller than or equal to 2 times of the number of the system cores. If the value is greater than 2 times of the number of the system cores, KWDB will read and write data based on 2 times of the number of the system cores. You can use this parameter in combination with the `auto_shrink` and `batch_rows` parameters, separating them using a comma (`,`). <br> **Note**: <br>- When enabling multi-thread import, if there is any line break character in the files to be imported, it will cause an import failure. In this case, you need to remove the databases or tables to be imported, set the `thread_concurrency` parameter to `1`, and then re-import the files.<br>- When enabling multi-thread import, if the file to be imported contains multiple data rows with the same timestamp for the same device, the write order becomes unpredictable. Under the default deduplication mode (`override`), the later writes overwrite existing data with the same timestamp, which may result in final import results that differ from expectations.|
...
| `limit_memory` | Optional. Limits the amount of data imported each time. By default, there is no limit. Supports pure numbers (bytes) or numbers with units. Units are case-insensitive, for example `100B`, `1024`, `1MB`. The maximum supported unit is PB. |
