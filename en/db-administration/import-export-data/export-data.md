...
During the data export process, if the target location is unreachable, the system returns an error. If the target path is not empty, the system returns an error: `export path %s is not empty (contains %d files), please use an empty directory to prevent data loss`, and the export operation is not executed. If the export is interrupted due to any other reason, the system retains the files that have already been successfully exported.
...
|---| --- |
| `expt_path` | The storage path for exported files. Supports two formats: `nodelocal://<node_id>/<dir>` and `<server_ip>/<dir>`.<br>`nodelocal://<node_id>/<dir>`: Exports files to the local node.<br> - `node_id`: The node ID. When there is only one node locally, `node_id` is `1`.<br> - `dir`: The name of the folder where exported data is stored. If the target folder does not exist, the system creates it under the KWDB data directory defined during KWDB installation. By default, the KWDB data directory is `/var/lib/kaiwudb/extern/<folder_name>`. If the target folder is not empty, the system returns an error: `export path %s is not empty (contains %d files), please use an empty directory to prevent data loss`.<br>`<server_ip>/<dir>`: Exports files to the specified server.<br> - `server_ip`: The IP address and port number of the server, for example, `http://172.18.0.1:8090`.<br> - `dir`: The name of the folder where exported data is stored. If the target folder does not exist, the system creates it. If the target folder is not empty, the system returns an error: `export path %s is not empty (contains %d files), please use an empty directory to prevent data loss`. |
