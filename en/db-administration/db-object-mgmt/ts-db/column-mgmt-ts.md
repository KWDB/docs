...
```sql
ALTER TABLE <table_name> ADD [COLUMN] [IF NOT EXISTS] <column_name> <data_type> [DEFAULT <default_expr> | NULL] [ENCODE '<encode_algo>'] [COMPRESS '<compress_algo>' [LEVEL '<level>']];
```
...
| `encode_algo` | Optional. Sets the column encoding algorithm. Matching is case-insensitive. Different data types support different encoding algorithms. For details, see [Data Compression](../../../db-operation/storage-mgmt.md#data-compression). Set to `disabled` to disable encoding for the column. If not specified, the data type default encoding is used. |
