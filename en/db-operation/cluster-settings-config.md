...
| `server.rest.infulxdb.batch_params`                                   | The schemaless-writing interface requires setting batch write parameters <batch_enabled>;<batch_size>;<workers>;<flush_interval>, with default values of `false;100;16;1000` <br>**Note**<br>`batch_enabled`indicates whether batch writing is enabled.<br> `batch_size`indicates the maximum number of rows per batch, which will be refreshed immediately once reached.<br>`workers`indicates the number of concurrent writing workers.<br>`flush_interval`indicates the scheduled refresh interval, with the unit being `ms`.                                                                                                                                                                                                                                                                                                         |   -        | string      |
...
| `sql.ts_insert_select.block_memory`                       | Memory limit for data blocks per write in time-series `INSERT INTO SELECT` statements. This parameter cannot be set to `0` | `200`       | int      |
...
| `ts.compress.last_segment.enabled` | Controls whether compression is enabled for the last segment. Supports the following values:<br>- `true`: Enable compression. Last segment data is compressed when written, which reduces memory and disk usage but increases CPU consumption during write operations.<br>- `false`: Disable compression. Last segment data is stored in raw format, which provides higher write performance but consumes more memory and disk space.<br>This parameter trades off between write performance and storage efficiency. In write-intensive scenarios, set this to `false` to improve write throughput; in storage-sensitive scenarios, set this to `true` to reduce resource consumption. | `true` | boolean |
| `ts.compress.stage` | Controls the compression mode for time-series data. Supports the following values:<br>- `0`: Disable encoding and compression.<br>- `1`: Enable encoding and disable compression.<br>- `2`: Disable encoding and enable compression.<br>- `3`: Enable encoding and compression.<br>**Note:** When upgrading from 3.1.x to 3.2.0, if you previously set `ts.compress.stage=2` manually, change it to `ts.compress.stage=3` after the upgrade. If you did not set it manually, no action is required. | `3` | int |
| `ts.compress.algorithm` | Sets the global default compression algorithm. Supports `lz4`, `zstd`, `zlib`, `snappy`, and `disabled`. It has lower priority than column-level custom compression settings. For more information, see [Data Compression](./storage-mgmt.md#data-compression). | `lz4` | string |
| `ts.compress.level` | Sets the global default compression level. Supports `low`, `medium`, and `high`. It has lower priority than column-level custom compression settings. For more information, see [Data Compression](./storage-mgmt.md#data-compression). | `medium` | string |
| `ts.dedup.rule`                                         | Data deduplication strategy. Supports the following parameters: <br>- `override`: Full row deduplication; later written data overwrites existing data with the same timestamp. <br>- `merge`: Deduplicate and consolidate data with the same timestamp. When data with the same timestamp is written multiple times, later written non-NULL column values overwrite the previously written corresponding column values, ultimately consolidating into a single row. This mode is suitable for scenarios where different fields with the same timestamp are written in batches. <br>- `discard`: Ignore newly written duplicate data and keep existing data. When duplicate data write fails, the client receives the count of successfully and unsuccessfully inserted data as a notice. | `override`  | string   |
| `ts.force_sync_file.enabled` | Controls whether to force disk synchronization (fsync) when writing time-series data:<br>- Enabled (`true`): Ensures data is physically written to disk before returning success. Guarantees recovery from power failures and crashes, but reduces write performance (greater impact on HDDs, moderate impact on SSDs)<br>- Disabled (`false`): Returns success after writing to system cache, significantly improving write performance. Allows recovery from crashes, but data in cache may be lost during power failure, requiring manual intervention to restart | `false` | bool |
| `ts.last_cache_size.max_limit` | Sets the memory limit for the last_row read cache of time-series data—that is, the cache memory allocated by each vgroup for storing the most recent data of each device.<br>The valid range is [0, 1,073,741,824] bytes, with a default of 1 GB (1,073,741,824 bytes).<br>Setting this to `0` disables the last_row read cache feature. When cache memory usage exceeds the configured limit, the system automatically evicts cached data. If you disable the read cache after enabling it, the system will evict all cached data on the first write operation, and subsequent writes will no longer be cached.<br>This parameter is best suited for scenarios with a moderate number of devices and frequent queries for the latest data. When the read cache is enabled, `last` and `last_row` queries can read data directly from memory, greatly reducing query response time.<br>**Note**: Using this feature in scenarios with a large number of devices may impact write performance. Increasing this parameter value can reduce cache eviction frequency and minimize the impact on write performance. It is recommended to adjust this parameter dynamically based on your actual workload to balance query performance and write performance. | `1073741824` | byte size |
...
| `ts.raft_log.sync_period` | Time-series data raft log flush period. The default `0s` means real-time flushing, while non-zero values trigger forced sync at specified intervals. Longer flush periods improve write performance, but if a node stops unexpectedly, up to one period's worth of data may be lost. Suitable for high-performance scenarios with relatively lower data consistency requirements. | `0s` | duration |
| `ts.raftlog_combine_wal.enabled` | Enables the raft log and WAL merge feature for time-series data. Once enabled, reduces write amplification and improves time-series data write performance.<br>**Notes:**<br>- Can only be set to `true` when no time-series tables exist. After time-series tables are created, it can be set to `false` but cannot be set back to `true`. Takes effect after restarting the cluster.<br>- After creating time-series tables and writing data, if you need to fully enable or disable WAL, set this parameter to `false`, configure the WAL level parameter, and restart the cluster.<br>- After a node goes offline due to machine failure (e.g., power outage), modifying this parameter is not recommended, as it may cause data loss. | `false` | bool |
...
| `ts.rows_per_block.max_limit`                           | Controls the maximum number of rows a single block can hold in an entity segment, preventing individual blocks from becoming too large and degrading read efficiency.<br> Value range is 1 to 50000.<br>This parameter balances storage density and query performance. Increasing this value improves compression ratios and saves storage space (suitable for storage-priority scenarios). Decreasing it reduces the amount of data scanned per query, improving query response times (suitable for query-priority scenarios).                                                                                                                                                                                                                             | `4096`      | int      |
| `ts.rows_per_block.min_limit`                           | Controls the minimum number of rows a single block can hold in an entity segment, serving as the row count threshold that triggers data merging from last segment to entity segment.<br> Value range is 1 to 50000.<br>This parameter ensures each block contains sufficient data rows to improve compression efficiency and storage density, avoiding excessive small blocks that lead to file fragmentation and increased metadata overhead.<br>Increasing this value reduces block count and metadata overhead but prolongs data merge time. Decreasing it accelerates data persistence but may generate more small blocks, increasing file fragmentation.                                                                                                                                             | `512`      | int      |
...
| `cluster.connection.timezone`               |Set the default time zone for the cluster.<br>Supports numeric offsets, for example:`（8,-5）`<br>Time zone names, for example:`（Asia/Shanghai）`<br>ISO8601 format, for example:`（+8:00）`<br>System table: `kwdb_internal.cluster_settings` <br>**Note**<br>Delete the original parameter:<br>`server.restful_service.default_reguest_timezone`<br>Replace the original parameter:<br>`server.restful_service.default_reguest_timezone`with a new parameter:<br> `cluster.connection.timezone`<br> After the replacement, when setting the global time zone for restful connections, the original time zone processing logic remains unchanged. | `UTC` | timestamp        |