...
| `ts.compression.level` | Controls the level of secondary compression, only effective when `ts.compression.stage` is set to`2`. Supports the following values:<br>- `low`: Low compression level, fast compression speed with lower compression ratio.<br>- `medium`: Medium compression level, achieving a balance between compression ratio and performance.<br>- `high`: High compression level, highest compression ratio but slower compression and decompression speeds.<br>**Note**: This parameter is not supported in the current version. | `medium` | string |
| `ts.compression.stage` | Controls the compression stage for time-series data. Supports the following values:<br>- `0`: No compression, data is stored in raw format.<br>- `1`: Primary compression, compresses columns using algorithms suited to their data types.<br>- `2`: Secondary compression, applies additional compression algorithms to further reduce storage space but increases CPU consumption.<br>This parameter trades off between storage space and computational resources. Higher compression stages can significantly reduce disk usage but will increase CPU overhead for both read and write operations. | `2` | int |
